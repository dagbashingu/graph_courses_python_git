...
```{python}
# Your code here
clean_loans = loans.drop(columns = ['rownames'])
clean_loans
```
...
demog_loan_info = clean_loans[['emp_title','emp_length','state','homeownership','loan_amount','term','interest_rate']]

# Renaming the term column to loan_term_months
demog_loan_info = demog_loan_info.rename( columns = {"term": "loan_term_months"})
demog_loan_info

# Export to CSV
demog_loan_info.to_csv('Outputs/demography_and_loan_Information.csv', index=False)
```
...
```{python}
# Your code here
credit_vars = clean_loans.filter(regex="credit")

# Exporting to csv
credit_vars.to_csv('Outputs/credit_variables.csv', index=False)
...
high_income = clean_loans.query("annual_income > 100000")
high_income.to_csv('Outputs/high_income_borrowers.csv', index=False)
```
...
```{python}
# Your code here
high_risk = clean_loans.query ("interest_rate > 15 | debt_to_income > 30 | delinq_2y == 1")
high_risk.to_csv('Outputs/high_risk_borrowers.csv', index=False)
...
```{python}
# Prime borrowers
prime = clean_loans.query("(grade == 'A'|grade == 'B') & debt_to_income < 20 & delinq_2y == 0")

prime.to_csv('Outputs/prime_borrowers.csv', index=False)
...
```{python}
# Combining all subsets in a single excel workbook on multiple sheets
import openpyxl 
