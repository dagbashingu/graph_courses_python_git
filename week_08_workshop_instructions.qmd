...
The average treatment cost is higher for Graduates
...
age_cost_table = tb.groupby("age_group").agg(patient_count = ("id", "size"), mean_total_cost = ("total_cost", "mean")
).round(2).reset_index()
age_cost_table
```
...
```{python}
# Your code here
px.bar(age_cost_table, y = "age_group", x="mean_total_cost")
```

Write a short interpretation of the plot.

# Insight
Patients within the age range of 18-36 incured the highest cost on health followed by the eldest category, 58+
...
delay_hf_type_table = (
    ...
    .agg(mean_numb_days=("treatment_initiation_delay_(in_days)", "mean"))
    .round(2)
    .reset_index()
)
delay_hf_type_table
...
```{python}
px.bar(delay_hf_type_table, y = "first_visit_type", x="mean_numb_days")
```
# Insight
From the plot we can deduce that there is more treatmt initiation delay in the public health facilities that the private health facilities though the difference doesn't look so significant.
