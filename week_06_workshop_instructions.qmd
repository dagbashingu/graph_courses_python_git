...
pa["mvpa_m_wkk"] = (pa["mvpa_s_day"] / 60) * 7
pa
```
...
```{python}
# your code here
pa["total_pa_m_wk"] = pa["light_ap_m_wk"] + pa["mvpa_m_wkk"]
pa
```
...
```{python}
# Vectorize function
tpa_cat_vec = np.vectorize(tpa_cat)
```

```{python}
pa["total_pa_cat"] = tpa_cat_vec(pa["total_pa_m_wk"])
pa
```

```{python}
pa["total_pa_cat"].value_counts()
```
...
```{python}
# your code here
tpam_wk = px.histogram(pa, x = "total_pa_m_wk")
tpam_wk.show()
```
...
```{python}
# your code here
px.box(pa, x = "status", y = "total_pa_m_wk")
```
...
The dataset has more Adults than children. The Adults engaged more in physical activities with a median of 718 minutes of physical activity per week as compared to a median 0f 642.3 minutes expended by the children.